...
- `# IVR BarberBook.txt` — **תכנון בלבד, טרם קיים קוד** — קביעת תור טלפונית (מענה קולי
  אוטומטי, Twilio, DTMF). כל ההחלטות שסוכמו + תסריט שיחה מלא + מבנה קוד מוצע. קרא במלואו
...
**קביעת תור טלפונית (IVR, 2026-08-03) — תכנון בלבד, אין קוד:** ראו `docs/# IVR BarberBook.txt`
לתכנון המלא (Twilio, DTMF, כל ההחלטות + תסריט שיחה). דורש תשתית שעדיין לא קיימת (דומיין+HTTPS,
חשבון Twilio) לפני שאפשר לעבור ל-production — פרטים במסמך.
