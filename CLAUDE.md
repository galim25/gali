...
- `# IVR BarberBook.txt` — **קוד נכתב (2026-08-04), טרם נבדק עם שיחה אמיתית** — קביעת
  תור טלפונית (מענה קולי אוטומטי, Twilio, DTMF). כל ההחלטות שסוכמו + תסריט שיחה מלא +
  סטטוס מימוש מעודכן. קרא במלואו לפני שממשיכים את הפיצ'ר הזה.
...
- ידוע ופתוח (לא תוקן, דורש החלטה אדריכלית): אין revocation לסשן קיים בעת reset סיסמה — JWT stateless ל-30 יום. אל תניחו שסשן "מבוטל" אחרי reset.
...
**קביעת תור טלפונית (IVR, 2026-08-03 תכנון / 2026-08-04 מימוש) — קוד נכתב, לא נבדק עם שיחה אמיתית:**
ראו `docs/# IVR BarberBook.txt` (סטטוס מעודכן בראש המסמך) — Twilio, DTMF, `apps/web/src/lib/ivr/`
(state machine + TwiML + אימות חתימה) ו-`apps/web/src/app/api/ivr/{voice,gather}/route.ts`. שתי
פונקציות ליבה חולצו כדי שהאפליקציה וה-IVR ישתפו אותה לוגיקה בלי כפילות: `bookAppointmentCore`
(`lib/actions/bookingCore.ts`) ו-`registerUserCore` (`lib/actions/registerCore.ts`) — שתיהן לא
`"use server"` בכוונה, כדי לקבל `actor`/פרטי הרשמה מפורשים במקום session. `pnpm build`/`pnpm
lint`/`pnpm test` עוברים נקי, אבל **אין עדיין חשבון Twilio/מספר טלפון/דומיין+HTTPS אמיתיים** —
עד שאלה יוקמו (או שמריצים ngrok לבדיקה מקומית, ראו סעיף 9 במסמך), אף שלב בתסריט השיחה לא הופעל
מול שיחה אמיתית. לפני production: לבדוק את כל התסריט קצה-לקצה.
